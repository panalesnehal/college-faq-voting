...
(function () {
  ...
  function render() {
    ...
    items.forEach(item => {
      ...
      const percent = Math.min(100, Math.round((votes / 15) * 100));
      const heightPx = Math.max(8, Math.round((percent / 100) * 72));
      bar.style.height = `${heightPx}px`;
      if (votes <= 5) bar.style.background = 'var(--green)';
      else if (votes <= 10) bar.style.background = 'var(--amber)';
      else bar.style.background = 'var(--red)';
      confusionWrap.appendChild(bar);
      const confusionLabel = document.createElement('div'); confusionLabel.className = 'confusion-label'; confusionLabel.textContent = 'Confusion Level';
      right.appendChild(confusionWrap); right.appendChild(confusionLabel);
      ...
      const countLabel = document.createElement('div'); countLabel.className = 'vote-helper'; countLabel.textContent = `${votes} students confused`;
      ...
      answer.innerHTML = '<div class="seen-badge">Seen by admin</div><div style="margin-top:8px;color:var(--muted)">Why students are confused (optional note placeholder)</div>';
      ...
      upBtn.addEventListener('click', async () => {
        ...
        // update UI
        countLabel.textContent = `${item.upvotes} students confused`;
        upBtn.disabled = true; upBtn.textContent = '✔ Marked as confusing';
        const helper = document.createElement('div'); helper.className = 'vote-helper'; helper.textContent = 'Thanks — this helps improve clarity.'; voteArea.appendChild(helper);
        // update confusion bar visuals
        const newPercent = Math.min(100, Math.round((item.upvotes / 15) * 100));
        const newHeight = Math.max(8, Math.round((newPercent / 100) * 72));
        bar.style.height = `${newHeight}px`;
        if (item.upvotes <= 5) bar.style.background = 'var(--green)';
        else if (item.upvotes <= 10) bar.style.background = 'var(--amber)';
        else bar.style.background = 'var(--red)';
      ...
      if (localStorage.getItem(`voted_${item.id}`)) { upBtn.disabled = true; upBtn.textContent = '✔ Marked as confusing'; }
